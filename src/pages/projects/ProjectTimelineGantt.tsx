...
function dateToMsSafe(input: any): number | null {
...
export default function ProjectTimelineGantt({
  ...
  const { minMs, rangeMs, todayX, rows, dateTicks } = useMemo(() => {
    const allDates: number[] = []

    const pushDate = (d: any) => {
      const ms = dateToMsSafe(d)
      if (ms !== null) allDates.push(ms)
    }

    pushDate(project?.createdAt)
    pushDate(project?.dueDate)

    milestones.forEach((m) => {
      pushDate(m.createdAt)
      pushDate(m.targetDate)
    })
    ...
    let minMs = Math.min(...allDates) - ONE_WEEK
    let maxMs = Math.max(...allDates) + ONE_WEEK

    const rangeMs = Math.max(maxMs - minMs, 86400000)
    const todayX = (today - minMs) / rangeMs

    const rows = milestones
      .map((m) => {
        const startMs = dateToMsSafe(m.createdAt)
        const endMs = dateToMsSafe(m.targetDate)
        ...
        return {
          ...
          endMs,
          workflowStateId: m.workflowStateId,
          targetDateLabel: new Date(endMs).toLocaleDateString(),
    ...
    const tickCount = 8
    const dateTicks = Array.from({ length: tickCount + 1 }).map((_, i) => {
      const ms = minMs + (rangeMs * i) / tickCount
      return { ms, ...formatTick(ms) }
    })
  ...
  const getProgress = (row: any) => {
    if (row.workflowStateId === doneStateId) return 1
    const now = Date.now()
    return Math.min(1, (now - row.startMs) / (row.endMs - row.startMs))
  }

  const { current } = Themestore()

  if (!milestones.length) {
    return <EmptyState title="No milestones" variant={'search'} />
  }

  return (
    <div className="flex flex-col w-full h-full" style={{ background: bg, color: current?.system?.dark }}>
      <div className="overflow-auto flex-1">
        <div style={{ minWidth: LABEL_WIDTH + CHART_MIN_WIDTH }}>

          {/* AXIS */}
          <div className="flex sticky top-0 z-10" style={{ background: bg }}>
            <div style={{ width: LABEL_WIDTH }} />
            <div className="flex-1 relative h-[60px]">
              {dateTicks.map((t, i) => {
                const x = (t.ms - minMs) / rangeMs
                return (
                  <div
                    key={i}
                  ...
                  >
                    <span className="text-[10px] opacity-60">{t.top}</span>
                    <span className="text-[11px] font-medium">{t.bottom}</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="flex">
            ...
            {/* LABELS */}
            <div style={{ width: LABEL_WIDTH, background: bg }}>
              {rows.map((r) => (
                <div
                  key={r.id}
                  style={{
            ...
            <div className="flex-1 relative" style={{ background: bg }}>
              ...
              {/* ROWS */}
              {rows.map((row, i) => {
                const start = (row.startMs - minMs) / rangeMs
                const end = (row.endMs - minMs) / rangeMs
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        style={{
                          background: getNoteBarBg(i, darkMode, fg),
                          borderRadius: 8,
                          padding: 8,
                          position: 'relative',
                      ...
                      <div className="relative mt-1 h-3">
                        {row.tasks.map((t: Task) => {
                          const ts = dateToMsSafe(t.createdAt)
                          const te = dateToMsSafe(t.dueDate)

                          if (ts === null || te === null) return null

                          const left = (ts - minMs) / rangeMs
                          const width = (te - ts) / rangeMs
              ...
              {hovered && (
                ...
                >
                  ...
                  <div>Due: {hovered.targetDateLabel}</div>
                  <div>{hovered.tasks.length} tasks</div>
                </div>
              )}
            </div>
          </div>
      ...
      {/* LEGEND */}
      <div className="p-2 flex items-center gap-2" style={{ background: bg }}>
        <div style={{ width: 2, height: 14, background: secondaryColor }} />
